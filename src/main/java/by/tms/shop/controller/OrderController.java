...
public class OrderController {
    ...
    public ModelAndView getOrderView(@PathVariable("id") long id, ModelAndView modelAndView) {
        ...
        if(order.getTechnicList().isEmpty()) {
            orderService.deleteOrderById(id);
            modelAndView.setViewName("redirect:/");
            log.info("orderController, getOrderView(Null) - success");
        } else {
            modelAndView.addObject("technics", order.getTechnicList());
    ...
    public ModelAndView postOrderPay(@PathVariable("id") long id, @ModelAttribute("price") String price, ModelAndView modelAndView) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        Order order = orderService.findOrderById(id);
        orderService.checkOrderForTechnicStatus(order);
        orderService.purchaseTechnicByUserIdAndOrderId(userService.findUserByLogin(authentication.getName()).getId(), id);
        for (Technic technic : order.getTechnicList()) {
            technicService.updateTechnicStatusById(technic.getId());
        }
    ...
    public ModelAndView postOrderRemove(@PathVariable("id") long id, long technicId, ModelAndView modelAndView) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if(!orderService.findOrderById(id).getOrderStatus().equals(OrderStatus.DELIVERED)) {
            orderService.removeTechnicFromOrderByIdAndUserLogin(id, technicService.findTechnicById(technicId), authentication.getName());
